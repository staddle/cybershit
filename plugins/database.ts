...
import { getDatabase, ref, child, get, set, push, onValue, connectDatabaseEmulator } from "firebase/database";
...
const getSeasons = async () : Promise<Season[]> => {
  ...
  const addSeason = (newSeason: Season) => {
    const newSeasonRef = push(ref(db, 'lol/seasons'))
  ...
  const addMatch = (match: Match, selectedSeasonId: string) : string => {
    const newMatchRef = push(ref(db, `lol/seasons/${selectedSeasonId}/matches`))
  ...
  const addParticipant = (name: string) : void => {
    const newParticipantRef = push(ref(db, 'lol/participants'))
  ...
  const refreshChampions = async () : Promise<void> => {
    ...
    await fetch('http://ddragon.leagueoflegends.com/cdn/12.6.1/data/en_US/champion.json')
      ...
      .then((res) => {
        ...
        const newChampionListRef = push(championDataRef)
        set(newChampionListRef, returnValue)
      })
  }
...
export default defineNuxtPlugin((nuxtApp) => {
  nuxtApp.provide('getSeasons', getSeasons )
  nuxtApp.provide('getSeason', getSeason )
  nuxtApp.provide('listenForSeason', listenForSeason )
  nuxtApp.provide('addSeason', addSeason )
  nuxtApp.provide('addMatch', addMatch )
  nuxtApp.provide('addParticipant', addParticipant )
  nuxtApp.provide('getParticipants', getParticipants )
  nuxtApp.provide('getChampionList', getChampionList )
  nuxtApp.provide('getChampion', getChampion )
  nuxtApp.provide('rollRoles', rollRoles )
  nuxtApp.provide('selectChampion', selectChampion )
})
